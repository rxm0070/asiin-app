...
writeFileSync('/tmp/student_qa_full.txt', text);

// Count questions
const qMatches = text.match(/问题\d+:/g);
console.log('Student Q&A count:', qMatches ? qMatches.length : 0);

// Print all questions
const questions = text.match(/问题\d+:[^\n]+/g);
if (questions) {
  console.log('\n--- All Questions ---');
