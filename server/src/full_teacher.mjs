...
console.log('Number of content items:', response.content.length);

// Print all content items with their types
response.content.forEach((item, idx) => {
  console.log(`\n--- Item ${idx} ---`);
  console.log('Type:', item.type);
  if (item.type === 'text') {
    console.log('Length:', typeof item.text === 'string' ? item.text.length : 'not string');
    console.log('Preview:', typeof item.text === 'string' ? item.text.substring(0, 200) : JSON.stringify(item.text).substring(0, 200));
  }
});
